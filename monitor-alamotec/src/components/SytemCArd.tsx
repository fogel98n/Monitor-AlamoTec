...
function SystemCard({ sistemas, onTaskUpdated, onEditar }: SystemCardProps) {
  ...
  // Se actualiza cada minuto para comprobar si una ejecución
  // ya superó los 30 minutos.
  const [, setTiempoActual] = useState(Date.now());
  ...
  const handleBorrar = async (sistema: SistemaConTareas) => {
    if (
      !confirm(
        `¿Eliminar el sistema "${sistema.nombre}"? Esta accion no se puede deshacer.`
      )
    ) {
      return;
    }
  ...
  // No permitir subir el primero
  if (direccion === -1 && index === 0) return;

  // No permitir bajar el último
  if (direccion === 1 && index === sistemas.length - 1) return;

  const sistema = sistemas[index];

  setCargando(`orden-${sistema.id}`);
    ...
    <div className="system-card">
      ...
      <div className="table-scroll">
        <table className="modules-table">
          <thead>
            <tr>
              <th className="col-basedatos" rowSpan={2}>
                Base de datos
              </th>
            ...
            <tr>
              {nombresModulos.map((nombre) => (
                <>
                  <th key={`${nombre}-e`} className="col-sub">
                    Estado
                  </th>

                  <th key={`${nombre}-u`} className="col-sub">
                    Última · dur.
                  </th>
                </>
              ))}
          ...
          <tbody>
            {sistemas.map((sistema, index) => (
              <tr key={sistema.id}>
                ...
                {sistema.error ? (
                  <td
                    colSpan={nombresModulos.length * 2}
                    className="col-sub error-sistema"
                  >
                    {sistema.error}
                  </td>
                ) : (
                  nombresModulos.map((nombreModulo) => {
                    const task = sistema.tareas.find(
                      (t) => t.name === nombreModulo
                    );
                    ...
                    // Calculamos cuánto tiempo lleva en ejecución.
                    const minutosEnEjecucion = task.laststart
                      ? Math.floor(
                          (Date.now() - task.laststart * 1000) / 60000
                        )
                      : 0;

                    // A partir de 30 minutos pasa a rojo.
                    const ejecucionMayor30Min =
                      isCaido && minutosEnEjecucion >= 30;

                    const statusText = isCaido
                      ? ejecucionMayor30Min
                        ? "en-ejecucion-rojo"
                        : "en-ejecucion"
                      : isActivo
                    ...
                    return (
                      <>
                        ...
                        >
                          <span
                            className={`pill pill-${statusText}`}
                          >
                            {statusText.startsWith("en-ejecucion")
                              ? "En ejecución"
                              : statusText}
                          </span>
                        </td>
                          ...
                          <div className="ultima-dur">
                            <span className="ultima-texto">
                              ...
                              {task.duracion_segundos !== null && (
                                <>
                                  {" "}
                                  ·{" "}
                                  <span className="dur-valor">
                                    {task.duracion_segundos} s
                                  </span>
                                </>
                              )}
                            </span>
                            ...
                            <div className="acciones-icono">
                              <button
                                ...
                                disabled={
                                  cargando === `toggle-${task.id}`
                                }
                                onClick={() =>
                                  handleToggle(sistema.id, task)
                                }
                              >
                                {isActivo ? (
                                  <IconPause />
                              ...
                              <button
                                ...
                                disabled={
                                  cargando === `reset-${task.id}`
                                }
                                onClick={() =>
                                  handleReset(sistema.id, task)
                                }
                              >
                                <IconRefresh />
                              </button>
                              ...
                              <button
                                className="icon-btn icon-btn-ejecutar"
